...
* **Precision is prioritised over speed**: These algorithms are intended to provide certificated outputs with precisely quantified error, and this is prioritised over runtime. Hence, whre possible, `Fraction` objects are used as opposed to `float`s so as to enable exact arithmetic. Complex numbers generated by class functions are typically given as tuples of `Fraction` objects, since they are known exactly. Complex numbers originating from user input are not expected to be exactly specified or exactly known and so are allowed to have `float`-type real and imaginary parts.
